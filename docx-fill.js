...
(function (global) {
  ...
  async function fillXml(xml, payload) {
    ...
    // Prefer an explicit balance from the form; fall back to computed
    let balance;
    const balanceRaw = get(payload, 'fees.balance', '');
    if (balanceRaw !== '' && !isNaN(parseFloat(balanceRaw))) {
      balance = parseFloat(balanceRaw);
    } else {
      balance = totalInvoice - deposit;
    }
