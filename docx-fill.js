...
(function (global) {
  ...
  function makeRun(value, bold, size) {
    bold = bold || false;
    // Font, colour, language, etc. all inherit from the paragraph and document
    // defaults so filled content matches the surrounding template. Only bold and
    // size are worth overriding, and size only when the caller specifies one.
    const boldXml = bold ? '<w:b/><w:bCs/>' : '';
    const szXml = size ? '<w:sz w:val="' + size + '"/><w:szCs w:val="' + size + '"/>' : '';
    const rPr = (boldXml || szXml) ? ('<w:rPr>' + boldXml + szXml + '</w:rPr>') : '';
  ...
  // Escape a label for use in a regex that runs against XML: also convert
  // any '&' character to the '&amp;' entity that Word writes for it.
  function labelPattern(labelText) {
    return reEscape(labelText).replace(/&/g, '&amp;');
  }
  ...
  function fillAfterLabel(xml, labelText, occurrence, value, bold, size) {
    ...
    const escaped = labelPattern(labelText);
  ...
  // Preserve text if the agent typed something non-numeric like "TBC".
  // Only apply currency formatting when the value actually parses as a number.
  function feeStr(raw) {
    if (raw === '' || raw === null || raw === undefined) return '';
    const cleaned = String(raw).replace(/[,\s\u20ac]/g, '');
    if (cleaned !== '' && !isNaN(Number(cleaned))) {
      return euro(parseFloat(cleaned));
    }
    return String(raw);
  }

  // Parse a fee input to a number for totals. Strips commas, spaces and euro
  // symbols first so "5,200", "€5200" and "5200" all become 5200. Returns 0
  // for empty or non-numeric text like "TBC".
  function feeNum(raw) {
  ...
  async function fillXml(xml, payload) {
    ...
    // === FEES ===
    // Keep the agent's raw text for the individual cell display (so "TBC" survives),
    // but parse to numbers for the total calculations.
    const feeCharterRaw = get(payload, 'fees.charter', '');
    const feeDeliveryRaw = get(payload, 'fees.delivery', '');
    const feeApaRaw = get(payload, 'fees.apa', '');
    const feeSecurityRaw = get(payload, 'fees.security', '');
    const feeDepositRaw = get(payload, 'fees.deposit', '');
    const feeBalanceRaw = get(payload, 'fees.balance', '');

    const charterFee = feeNum(feeCharterRaw);
    const delivery = feeNum(feeDeliveryRaw);
    const apa = feeNum(feeApaRaw);
    const security = feeNum(feeSecurityRaw);
    const totalCharter = charterFee + delivery;
    const totalInvoice = totalCharter + apa + security;
    const deposit = feeNum(feeDepositRaw);
    // Balance: honour explicit form entry (numeric or text). Only compute if blank.
    const balanceDisplay = feeBalanceRaw !== ''
      ? feeStr(feeBalanceRaw)
      : euro(totalInvoice - deposit);

    xml = fillAfterLabel(xml, 'Charter Fee:', 0, feeStr(feeCharterRaw), false, '20');
    xml = fillAfterLabel(xml, 'Delivery & Re-Delivery Fees:', 0, feeStr(feeDeliveryRaw), false, '20');
    xml = fillAfterLabel(xml, 'Total Charter Fee:', 0, euro(totalCharter), true, '20');

    // APA cell — the template uses actual curly quote characters (U+201C, U+201D),
    // not the &#x201C; / &#x201D; entities. PizZip decodes numeric entities on read.
    ...
    if (apaPos >= 0) {
      ...
      if (result) {
        const mm = result.match;
        const run = makeRun(feeStr(feeApaRaw));
        if (result.style === 'wrap') {
    ...
    xml = fillAfterLabel(xml, '5% Security Deposit Against Damages:', 0, feeStr(feeSecurityRaw), false, '20');
    xml = fillAfterLabel(xml, 'Total Amount to Invoice:', 0, euro(totalInvoice), true, '20');
    xml = fillAfterLabel(xml, 'Deposit Amount:', 0, feeStr(feeDepositRaw), true, '20');
    xml = fillAfterLabel(xml, 'Deposit Amount due Date:', 0, get(payload, 'fees.deposit_due'), true, '20');
    xml = fillAfterLabel(xml, 'Balance Payment:', 0, balanceDisplay, true, '20');
    ...
    // === SIGNATURES ===
    // In the original template, the label + underscores are usually one run,
    // but Word sometimes splits them into 2 runs (label with N underscores,
    // then another run with a few more). The regex captures both by allowing
    // an optional trailing underscore-only run right after the label run.
    const sigNames = [
    ...
    const trailingUnderscoreRun = /(?:<\/w:r>\s*<w:r\b[^>]*>(?:<w:rPr>(?:[^<]|<(?!\/w:rPr>))*<\/w:rPr>)?<w:t[^>]*>_+<\/w:t>)*/;
    for (const name of sigNames) {
      const re = new RegExp(
        '<w:t([^>]*)>(FULL NAME OF SIGNATORY:\\s*)_+</w:t>' + trailingUnderscoreRun.source
      );
      const m = xml.match(re);
      if (!m) break;
      // Replace the entire match with just label + name in the first run;
      // trailing underscore-only runs get their underscores stripped so
      // they render as empty spans (harmless).
      const trailing = m[0].slice(m[0].indexOf('</w:t>') + '</w:t>'.length);
      const trailingCleaned = trailing.replace(/<w:t[^>]*>_+<\/w:t>/g, '<w:t></w:t>');
      const rep = '<w:t' + m[1] + '>' + m[2] + escapeXml(name) + '</w:t>' + trailingCleaned;
    ...
    for (let i = 0; i < sigNames.length; i++) {
      const re = new RegExp(
        '<w:t([^>]*)>(DATE:\\s*)_+</w:t>' + trailingUnderscoreRun.source
      );
      const m = xml.match(re);
      if (!m) break;
      const trailing = m[0].slice(m[0].indexOf('</w:t>') + '</w:t>'.length);
      const trailingCleaned = trailing.replace(/<w:t[^>]*>_+<\/w:t>/g, '<w:t></w:t>');
      const rep = '<w:t' + m[1] + '>' + m[2] + escapeXml(sigDate) + '</w:t>' + trailingCleaned;
      xml = xml.slice(0, m.index) + rep + xml.slice(m.index + m[0].length);
    }
    ...
    // Preference Sheet: Further Requirements box.
    xml = fillAfterLabel(xml, 'SHOULD YOU HAVE FURTHER REQUIREMENTS INCLUDE CLEARLY IN THIS BOX', 0,
      get(payload, 'further_requirements'), false, '16');

    // Preference Sheet: Time of Charter — Start/Finish times when the "Other"
    // slot is selected. Labels are "Start:  ____________" and "Finish: ____________".
    const timeSlot = get(payload, 'time_slot', 'full_day');
    const fromTimeForSheet = get(payload, 'charter.from_time');
    const toTimeForSheet = get(payload, 'charter.to_time');
    if (timeSlot === 'other' && (fromTimeForSheet || toTimeForSheet)) {
      const startRe = /<w:t([^>]*)>Start:\s*_+<\/w:t>/;
      const startMatch = xml.match(startRe);
      if (startMatch && fromTimeForSheet) {
        const rep = '<w:t' + startMatch[1] + ' xml:space="preserve">Start:  ' + escapeXml(fromTimeForSheet) + '</w:t>';
        xml = xml.slice(0, startMatch.index) + rep + xml.slice(startMatch.index + startMatch[0].length);
      }
      const finishRe = /<w:t([^>]*)>Finish:\s*_+<\/w:t>/;
      const finishMatch = xml.match(finishRe);
      if (finishMatch && toTimeForSheet) {
        const rep = '<w:t' + finishMatch[1] + ' xml:space="preserve">Finish: ' + escapeXml(toTimeForSheet) + '</w:t>';
        xml = xml.slice(0, finishMatch.index) + rep + xml.slice(finishMatch.index + finishMatch[0].length);
      }
    }

    // Menu order prompt on the Preference Sheet
    xml = fillAfterLabel(
      xml,
      'ASK OUR TEAM TO PROVIDE YOU WITH OUR MENUS THAT WERE CURATED FOR YACHT CHARTERS. ORDER HERE',
      0, get(payload, 'menu_order'), false, '16'
    );
    ...
    // Main contact — only fill from explicit main_contact.* values. No auto-fallback
    // to charterer or agent details; empty here means empty on the sheet.
    xml = fillAfterLabel(xml, 'Full Name:', 1, get(payload, 'main_contact.name'), false, '16');
