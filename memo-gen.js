...
(function (global) {
  ...
  const MEMO_STYLES = `
    ...
    * { box-sizing: border-box; }
    ...
    .signoff { margin-top: 56px; padding-top: 24px; border-top: 1px solid var(--navy-10); }
    .signoff__grid { display: grid; grid-template-columns: 1fr; gap: 40px; }
    @media (min-width: 620px) {
      .signoff__grid { grid-template-columns: 1fr 1fr; gap: 48px; }
    }
    .signoff__line { border-bottom: 1px solid var(--navy-30); padding-bottom: 4px; height: 64px; }
    ...
    @media print {
      ...
      .header { padding-bottom: 14px; margin-bottom: 22px; }
      .brand { font-size: 13px; }
      .section { margin-bottom: 20px; page-break-inside: avoid; break-inside: avoid; }
      .section__title { font-size: 10px; margin-bottom: 10px; padding-bottom: 6px; }
      .data-grid dt { font-size: 9px; margin-top: 10px; }
      .data-grid dd { font-size: 12px; }
      @media (min-width: 620px) {
        .data-grid { column-gap: 24px; row-gap: 12px; }
        .data-grid dt { font-size: 10px; margin-top: 0; padding-top: 2px; }
      ...
      .signoff__grid { gap: 48px; }
      .signoff__line { height: 64px; }
...
'  <button class="tb-btn tb-btn--ghost" id="tbWord">Save as Word</button>' +
...
'      a.href = url;' +
'      a.download = (document.title || "Charter Memorandum").replace(/[\\\\/:*?\\"<>|]/g, "") + ".doc";' +
...
((get(payload, 'special_occasion') || get(payload, 'notes.crew_internal'))
  ? '<section class="section">' +
...
'    <div><div class="signoff__line"></div><div class="signoff__label">Received by \u00b7 ' +
      esc(get(payload, 'crew.captain_name') || '(Captain)') + '</div></div>' +
'  </div>' +
'</div>' +
