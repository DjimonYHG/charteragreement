...
'  <dl class="data-grid">' +
    ...
    toggleRow('Catering Supplier', catererEnabled,
      catererEnabled === true
        ? ((catererName || 'Details not filled in') +
           (catererContact ? ' \u00b7 ' + catererContact : ''))
        : null) +
    (get(payload, 'menu_order')
      ? row('Menu Order from Yachthub', get(payload, 'menu_order'))
      : '') +
    (menuCount ? row('Number of Menus Ordered', String(menuCount)) : '') +
    menusHtml +
'  </dl>' +
    (menuImage
      ? '<div style="margin-top:14px;"><div style="font-size:10px;text-transform:uppercase;letter-spacing:0.08em;color:var(--navy-50);margin-bottom:6px;">Menu Image</div>' +
