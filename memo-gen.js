...
'  <h2 class="section__title">Food</h2>' +
    ...
    // Chef on Board — details block only when confirmed
    (chefConfirmed
      ? '  <div style="margin-bottom:14px;">' +
        '    <div style="font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:0.08em;color:var(--navy);margin-bottom:6px;">Chef on Board</div>' +
        '    <dl class="data-grid">' +
             (chefName ? row('Chef name', chefName) : '') +
             (chefContact ? row('Chef contact', chefContact) : '') +
             (chefNotes ? row('Chef notes', chefNotes) : '') +
             (!chefName && !chefContact && !chefNotes ? '<dt>Status</dt><dd>Confirmed</dd>' : '') +
        '    </dl>' +
        '  </div>'
      : '  <dl class="data-grid">' + row('Chef on Board', 'No') + '</dl>') +

    // Outside Caterer — details block only when Yes; explicit No or unset shown as toggle row
    (catererEnabled === true
      ? '  <div style="margin-bottom:14px;">' +
        '    <div style="font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:0.08em;color:var(--navy);margin-bottom:6px;">Outside Caterer</div>' +
        '    <dl class="data-grid">' +
             (catererName ? row('Caterer name', catererName) : row('Caterer name', '', 'Details not filled in')) +
             (catererContact ? row('Caterer contact', catererContact) : '') +
             (catererNotes ? row('Caterer notes', catererNotes) : '') +
        '    </dl>' +
        '  </div>'
      : '  <dl class="data-grid">' + toggleRow('Outside Caterer', catererEnabled, null) + '</dl>') +

    // Menu order from Yachthub — always visible when there's content
    (menuOrder
      ? '  <div style="margin-bottom:14px;">' +
        '    <div style="font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:0.08em;color:var(--navy);margin-bottom:6px;">Menu order from Yachthub</div>' +
        '    <dl class="data-grid">' + row('Order', menuOrder) + '</dl>' +
        '  </div>'
      : '') +

    // Shared menu details — count, blocks, image
    ((menuCount || menusHtml || menuImage)
      ? '  <div>' +
        '    <div style="font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:0.08em;color:var(--navy);margin-bottom:6px;">Menu details</div>' +
        '    <dl class="data-grid">' +
